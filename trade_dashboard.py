...
RAW_DATASET = "raw_trade_data"
COMBINED_DATASET = "combined_trade_data"
PROCESSED_DATASET = "processed_trade_data"
ANALYTICS_DATASET = "trade_analytics"
...
selected_year = st.sidebar.selectbox("Select Year", available_years, index=available_years.index(latest_year) if latest_year in available_years else 0)
...
if page == "Overview":
    ...
    if not global_metrics_df.empty:
        ...
        if selected_year_metrics is not None:
            ...
            with col1:
                st.metric(
                    label="Global Exports (USD)",
                    value=f"${selected_year_metrics['global_exports']/1e9:.2f}B",
                    delta=f"{(selected_year_metrics['global_exports'] - prev_year_metrics['global_exports'])/prev_year_metrics['global_exports']*100:.1f}%" if prev_year_metrics is not None else None
                )
            
            # Global Imports
            with col2:
                st.metric(
                    label="Global Imports (USD)",
                    value=f"${selected_year_metrics['global_imports']/1e9:.2f}B",
                    delta=f"{(selected_year_metrics['global_imports'] - prev_year_metrics['global_imports'])/prev_year_metrics['global_imports']*100:.1f}%" if prev_year_metrics is not None else None
                )
            
            # Average ECI
            with col3:
                st.metric(
                    label="Average Economic Complexity Index",
                    value=f"{selected_year_metrics['avg_eci']:.2f}",
                    delta=f"{selected_year_metrics['avg_eci'] - prev_year_metrics['avg_eci']:.2f}" if prev_year_metrics is not None else None
                )
            ...
            fig = px.line(
                global_metrics_df,
                x="year",
                y=["global_exports", "global_imports"],
                labels={"value": "Trade Volume (USD)", "year": "Year", "variable": "Type"},
                title="Global Trade Volume Over Time",
                color_discrete_map={"global_exports": "#1f77b4", "global_imports": "#ff7f0e"}
            )
...
# Economic Complexity vs. Trade Balance page
    ...
    WHERE year = {selected_year}
    """
    eci_balance_df = load_data(eci_balance_query)
...
# Partner Diversification page
    ...
    """
    diversification_df = load_data(diversification_query)
    ...
    if not diversification_df.empty:
        ...
        diversification_df['diversification_score'] = 1 - diversification_df['partner_hhi']  # Higher is better
        diversification_df['combined_score'] = (diversification_df['diversification_score'] + 
                                              (diversification_df['eci'] - diversification_df['eci'].min()) / 
                                              (diversification_df['eci'].max() - diversification_df['eci'].min())) / 2
        
        # Sort countries by combined score
        top_countries = diversification_df.sort_values('combined_score', ascending=False).head(20)['country_id'].tolist()
        filtered_df = diversification_df[diversification_df['country_id'].isin(top_countries)]
        ...
        concentration_counts = diversification_df['concentration_category'].value_counts().reset_index()
